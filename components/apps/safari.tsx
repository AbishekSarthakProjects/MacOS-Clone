...
export function Safari() {
  const [url, setUrl] = useState("https://www.pineapple-os.local")
  const [tabs, setTabs] = useState([
    { id: 1, title: "Pineapple", url: "https://www.pineapple-os.local", active: true, favicon: "P" },
    { id: 2, title: "GitHub", url: "https://github.com", active: false, favicon: "G" },
  ])
  const [activeTab, setActiveTab] = useState(1)

  const favorites = [
    { name: "Pineapple", color: "from-gray-700 to-gray-900", favicon: "P" },
    { name: "GitHub", color: "from-gray-800 to-black", favicon: "G" },
    { name: "Twitter", color: "from-blue-400 to-blue-600", favicon: "X" },
    { name: "YouTube", color: "from-red-500 to-red-700", favicon: "Y" },
    { name: "Reddit", color: "from-orange-500 to-red-600", favicon: "R" },
    { name: "Netflix", color: "from-red-600 to-black", favicon: "N" },
    { name: "Amazon", color: "from-yellow-400 to-orange-500", favicon: "A" },
    { name: "LinkedIn", color: "from-blue-600 to-blue-800", favicon: "in" },
  ]
  ...
  const addTab = () => {
    const newTab = {
      id: Date.now(),
      title: "New Tab",
      url: "https://www.pineapple-os.local",
      active: false,
      favicon: "P",
    }
    setTabs([...tabs, newTab])
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="h-10 bg-[#e8e8e8] border-b border-gray-300 flex items-center px-2 gap-1">
        {tabs.map((tab) => (
          ...
          >
            <span className="flex h-4 w-4 items-center justify-center rounded bg-gray-200 text-[10px] font-semibold text-gray-700">
              {tab.favicon}
            </span>
      ...
      <div className="h-10 bg-[#fafafa] border-b border-gray-200 flex items-center px-4 gap-4 overflow-x-auto">
        {favorites.map((fav) => (
          ...
          >
            <span className="flex h-5 w-5 items-center justify-center rounded bg-gray-200 text-[10px] font-semibold text-gray-700">
              {fav.favicon}
            </span>
          ...
          <div className="text-center space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mt-16">
              {favorites.map((site) => (
                ...
                >
                  <div
                    className={`flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br ${site.color} text-xl font-semibold text-white shadow-sm md:h-16 md:w-16 md:text-2xl`}
                  >
                    {site.favicon}
                  </div>
